...
class TestOrderConfirmationView(TestCase):
    """Tests for verifying the behavior of the order confirmation view."""
    ...
    def test_order_confirmation_view_uses_correct_template_when_no_order(self):
        """Test that the order confirmation view renders the expected template
        when there is no order."""
        response = self.client.get(self.order_confirmation_url)
        self.assertTemplateUsed(response, "orders/order-confirmation.html")

    def test_order_context_is_set_to_none_when_no_order(self):
        """
        Test that order context is set to none when there is no order.
        """
        response = self.client.get(self.order_confirmation_url)
        self.assertIsNone(response.context["order"])

    def test_order_create_post_uses_correct_template(self):
        """
        Test that the order confirmation view renders the expected template.
        """
        self.client.post(self.cart_add_url, {"offer_id": 1, "action": "post"}, xhr=True)
    ...
    def test_order_confirmation_view_context_contains_order(self):
        """Test that the order confirmation view context includes the created order."""
        self.client.post(self.cart_add_url, {"offer_id": 1, "action": "post"}, xhr=True)
        self.client.post(self.order_create_url)
    ...
    def test_order_confirmation_view_context_contains_solo_ticket(self):
        """Test that the order confirmation view context the Solo ticket."""
        self.client.post(self.cart_add_url, {"offer_id": 1, "action": "post"}, xhr=True)
        self.client.post(self.order_create_url)
        response = self.client.get(self.order_confirmation_url)
        self.assertIn("Solo", response.context["tickets_by_offer"])
